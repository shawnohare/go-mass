...
// A Partition of a Collection interface into cells.
...
func MakePartition(c Collection, partition []int) (*Partition, error) {
	var err error
...
func makeCells(c Collection, partition []int) ([]Slice, error) {
...
func makePoints(c Collection, par []int) []float64 {
...
func MakeEqualWidthCells(c Collection, n int) (*Partition, error) {
...
func DefineEqualWidthCells(c Collection, n int) (*Partition, error) {
...
func MakeMinSizeCells(c Collection, n, k int) (*Partition, error) {
...
// input Collection into n subinterval cells each of which has at least
...
func DefineMinSizeCells(c Collection, n, k int) (*Partition, error) {
	N := c.Len()
	if n < 1 || k < 0 || N < n*k || N < 2 || c.Mass(N-1) == c.Mass(0) {
		err := errors.New("Collection cannot be partitioned.")
		return nil, err
	}
